...
```
AIStroke/
├── Test-And-Record-Demand.txt         # 测试与记录需求说明
├── aistroke/
│   ├── CT_CV_main.py                  # 主训练/验证入口
│   ├── main.py                        # 训练主程序
│   ├── stage1.py                      # 阶段一训练脚本
│   ├── config/                        # 配置模块
│   ├── data/                          # 数据处理与特征生成
...
- 其他依赖请参考 `requirements.txt` 或根据实际报错安装

## 快速开始
1. **数据准备**
   - 将原始数据放入 `raw_data_generate/`，运行相关脚本生成训练所需特征。
2. **配置参数**
   - 修改 `aistroke/config/config.py` 以适配本地路径和参数。
3. **训练模型**
   - 进入 `aistroke/` 目录，运行：
     ```bash
     python main.py
...
## 训练与推理流程
1. 数据采集与预处理（`raw_data_generate/`）
2. 特征生成与增强（`aistroke/data/`）
3. 配置参数（`aistroke/config/`）
4. 模型训练与验证（`aistroke/main.py`、`CT_CV_main.py`）
5. 日志与模型保存（`aistroke/logs/`）
6. 推理与评估（可扩展）
